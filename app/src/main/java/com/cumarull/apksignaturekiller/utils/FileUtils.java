...
public class FileUtils {
    ...
    public static void assembleSmaliString(@NotNull String src, @NotNull DexBuilder dexBuilder) throws Exception {
        int apiLevel = 20;
        smaliFlexLexer lexer = new smaliFlexLexer(new StringReader(src), apiLevel);
        lexer.setSourceFile(null);
        ...
        if (parser.getNumberOfSyntaxErrors() > 0 || lexer.getNumberOfSyntaxErrors() > 0) {
            throw new IOException("Parse smali failed");
        }
        CommonTreeNodeStream treeStream = new CommonTreeNodeStream(result.getTree());
